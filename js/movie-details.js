...
function createUserCard(user) {
    ...
    const avatarSrc = user.avatar_url ? `https://framecodefs.onrender.comkend.onrender.com${user.avatar_url}` : 'assets/user icon.png';
...
async function renderReviews(page = 1) {
    ...
    try {
        const response = await fetch(`https://framecodefs.onrender.comkend.onrender.com/api/reviews/${movieId}?page=${page}&limit=${reviewsPerPage}`, { headers });
        ...
        reviewsForThisMovie.forEach(review => {
            ...
            const avatarSrc = review.avatar_url ? `https://framecodefs.onrender.comkend.onrender.com${review.avatar_url}` : 'assets/user icon.png';
...
function updateReviewsPagination(currentPage, totalPages) {
    ...
    reviewForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('https://framecodefs.onrender.comkend.onrender.com/api/reviews', {
                method: 'POST',
    ...
    async function checkFavoriteStatus() {
        ...
        try {
            const res = await fetch('https://framecodefs.onrender.comkend.onrender.com/api/favorites/ids', { headers: { 'x-auth-token': token } });
    ...
    favoriteBtn.addEventListener('click', async () => {
        ...
        const url = `https://framecodefs.onrender.comkend.onrender.com/api/favorites/${isFavorited ? movieId : ''}`;
