document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchUsers(page = 1, search = '') {
        try {
            const response = await fetch(`https://framecodefs.onrender.com/api/users?page=${page}&limit=10&search=${search}`, { headers });
    ...
    async function fetchAllReviews(page = 1, search = '') {
        try {
            const response = await fetch(`https://framecodefs.onrender.com/api/reviews/all/reviews?page=${page}&limit=10&search=${search}`, { headers });
            const data = await response.json();
    ...
    reviewsList.addEventListener('click', async (e) => {
        if (e.target.classList.contains('delete-review-btn')) {
            ...
            if (confirm(`ADMIN: Tem certeza que deseja excluir a avaliação #${reviewId}?`)) {
                try {
                    const response = await fetch(`https://framecodefs.onrender.com/api/reviews/${reviewId}`, {
                        method: 'DELETE',
                        headers: headers
                    });
                    const data = await response.json();
